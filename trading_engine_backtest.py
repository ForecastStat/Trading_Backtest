...
class IntegratedRevolutionaryTradingEngine:
    ...
    # In trading_engine_backtest.py, sostituisci __init__

    def __init__(self, capital, open_positions, performance_db_path):
        
        # === CONFIGURAZIONE SISTEMA PER BACKTEST ===
        self.capital = capital
        self.open_positions = open_positions
        self.trade_history = [] # La cronologia è gestita dall'orchestratore
        self.daily_pnl = []
        self.portfolio_value_history = []
    
        # Imposta il percorso del DB AI passato dall'orchestratore
        # Questo è FONDAMENTALE per l'apprendimento persistente
        global PERFORMANCE_DB_FILE
        PERFORMANCE_DB_FILE = Path(performance_db_path)
        ...
        self.logger = logging.getLogger(f"{__name__}.IntegratedEngineBacktest")
        
        # File paths (adattati per il backtest)
        self.analysis_data_file = DATA_DIR / "latest_analysis.json"
        self.parameters_file = DATA_DIR / "optimized_parameters.json"
        ...
        self.ai_enabled = True # L'AI è sempre attiva, la sua logica bootstrap gestirà l'apprendimento
        self.meta_orchestrator = MetaOrchestrator()
        self.real_alpha_research_enabled = False # Disabilitato per il backtest per semplicità
        ...
        self.logger.info("🧠 INTEGRATED AI SYSTEM INITIALIZED FOR BACKTEST!")
    ...
    def load_state(self):
        """
        Versione Backtest: Disattivata. Lo stato è passato tramite __init__.
        Registra comunque i trade storici dell'orchestratore nel DB dell'AI.
        """
        self.logger.info("Backtest: load_state() chiamato, ma lo stato è gestito dall'orchestratore.")
    ...
    def save_state(self):
        """
        Versione Backtest: Disattivata. Lo stato è gestito dall'orchestratore.
        """
        # Non fare nulla, l'orchestratore si occupa dello stato.
        return True
    ...
    def detect_overall_market_trend(self, sp500_data):
        ...
        try:
            ...
            if last_close > last_sma20 and last_sma20 > last_sma50:
                ...
                regime = "strong_bear" if is_low_vol else "volatile_bear"
            ...
            self.logger.error(f"Errore (backtest) nel rilevare trend di mercato: {e}")
            return 'unknown'
...
<p>System Status: Operational with limitations</p>
    ...
    def run_integrated_trading_session_for_backtest(self, analysis_data_path, sp500_data_full, current_backtest_date):
        ...
        try:
            ...
            # 1. Caricamento dati di analisi (legge il file generato dallo Stock Analyzer)
            ...
            # 2. Rileva il trend di mercato usando i dati storici fino alla data corrente
            ...
            if self.ai_enabled and self.meta_orchestrator:
                # Conta i trade chiusi nel DB AI per la logica di bootstrap
                try:
                    with sqlite3.connect(self.meta_orchestrator.performance_learner.db_path) as conn:
                        cursor = conn.execute('SELECT COUNT(*) FROM trades WHERE exit_date IS NOT NULL')
                ...
                # Controlla se è necessario fare il training
                ...
                if (not self.meta_orchestrator.performance_learner.is_trained or
                    ...
                    print("--- Tentativo di training AI in corso... ---")
            ...
            # 5. Prepara i segnali per l'esportazione JSON (non esegue trade)
            ...
            # NOTA: Assicurati che EXECUTION_SIGNALS_FILE sia definito globalmente o passato come argomento
            # Per ora, si assume che sia accessibile come nel codice originale.
        ...
        # NOTA: Il blocco 'except' inizia qui, allo stesso livello del 'try'
        except Exception as e:
            self.logger.error(f"ERRORE CRITICO nella sessione di trading (Backtest): {e}")
            import traceback
            self.logger.error(traceback.format_exc())
            return False
