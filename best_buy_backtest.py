...
def run_one_time_screening_for_backtest():
    """
    Versione per il backtest.
    Restituisce una lista statica di ticker robusti da analizzare per tutto l'anno.
    Questa funzione sostituisce la chiamata live a Finviz.
    """
    print("Esecuzione screening una tantum per il backtest...")
    
    # Per il 2015, usiamo una lista rappresentativa di titoli blue-chip e popolari
    # che erano sicuramente liquidi e disponibili. Questo è più robusto che
    # tentare di replicare uno screener di 8 anni fa.
    # Ho preso questi ticker dal tuo `sector_universe` originale.
    
    static_tickers = [
        # Technology
        'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'ORCL', 'CSCO', 'CRM', 'ADBE', 'NFLX',
        # Healthcare
        'JNJ', 'PFE', 'UNH', 'ABBV', 'TMO', 'MRK', 'AMGN', 'GILD', 'BIIB',
        # Financials
        'JPM', 'BAC', 'WFC', 'C', 'GS', 'MS', 'AXP', 'BLK',
        # Consumer Discretionary
        'HD', 'NKE', 'SBUX', 'LOW', 'TJX', 'MCD', 'DIS',
        # Energy
        'XOM', 'CVX', 'COP', 'SLB', 'EOG',
        # Industrials
    ...
    unique_tickers = sorted(list(set(static_tickers)))
    
    print(f"Screening completato. {len(unique_tickers)} titoli statici selezionati per il backtest.")
